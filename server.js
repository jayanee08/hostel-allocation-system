...
app.get('/api/notifications/:userId', async (req, res) => {
    try {
        ...
        const result = await request
            .input('userId', sql.Int, userId)
            .query(`SELECT b.Status, b.RejectionReason, r.BlockName, r.FloorNumber, r.RoomNumber, b.BookingDate
                    FROM Bookings b 
                    JOIN Rooms r ON b.RoomID = r.RoomID 
                    WHERE b.UserID = @userId AND b.Status = 'Rejected' AND b.RejectionReason IS NOT NULL
...
app.post('/api/toggle-room-status', async (req, res) => {
    try {
        ...
        // If changing from Occupied to Available, reject the student's booking
        if (newStatus === 'Available') {
            await pool.request()
                .input('roomId', sql.Int, roomId)
                .query('UPDATE Bookings SET Status = \'Rejected\', RejectionReason = \'Your room booking is rejected by admin and made as Available. Kindly book another room. Contact admin for further assistance\' WHERE RoomID = @roomId AND Status = \'Approved\'');
        }
