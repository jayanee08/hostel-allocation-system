    ...
    try {
        ...
        const result = await request
            ...
            .query(`SELECT b.*, r.BlockName, r.FloorNumber, r.RoomNumber, r.RoomType, r.ACType,
                    CASE WHEN b.Status = 'Rejected' AND b.RejectionReason IS NOT NULL 
                         THEN b.RejectionReason 
                         ELSE CASE WHEN b.Status = 'Rejected' THEN 'Room already occupied' ELSE NULL END 
                    END as RejectionMessage
                    FROM Bookings b 
...
app.post('/api/toggle-room-status', async (req, res) => {
    try {
        ...
        if (newStatus === 'Available') {
            await pool.request()
                .input('roomId', sql.Int, roomId)
                .query(`UPDATE Bookings SET Status = 'Rejected', RejectionReason = 'Your room is rejected by admin, Try booking another room' 
